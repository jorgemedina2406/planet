<div class="col">
    ...
    <select class="form-control kt-selectpicker" name="patent_id" id="patent" data-live-search="true">
        ...
        <option value="{{ $patent->id }}" data-xyz="{{ $patent->agent_aduanal }}" {{ ($patent->id == $export->patent_id) ? 'selected' : '' }}>{{ $patent->patent }}</option>
        @endforeach
    </select></div>
<div class="col">
    <label class="form-control-label">Agente Aduanal:</label>
    @php
        $agent = (isset($export->patent)) ? $export->patent->agent_aduanal : '';
    @endphp
