...
class ExportsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    ...
    */
    public function collection()
    {
        ...
        foreach( $exports as $export) {
            ...
            $cards = CardsExports::select(\DB::raw('count(*) as num'))->groupBy('cards_exports.export_id')->get();

            \Log::debug('AQUI EN qeqweqwqweqeq', [$export->id]);

            if( count($export->cards) < $cards->max()->num )
            {
                foreach( $export->cards as $key => $item ) {
                    $all['card_name_'.$key]     = $item->name;
                }

                for ($i=1; $i <= ($cards->max()->num - count($export->cards))  ; $i++) { 
                    $all['card_name_'.$i] = '';
                }
            ...
            foreach( $pediment as $llave => $pedi ) {
                \Log::debug('AQUI EN asdasdasd', ['en asdasdasd']);
    ...
    public function headings(): array
    {
        ...
        \Log::debug('AQUI EN heading', ['en heading']);
        ...
        for ($i=1; $i <= $cards->max()->num ; $i++) {
            array_push($info, 'Carta '.$i);
        }
    ...
    public static function afterSheet(AfterSheet $event)
    {
        ...
        \Log::debug('AQUI EN AFTER', ['en after']);
