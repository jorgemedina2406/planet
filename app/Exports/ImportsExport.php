...
class ImportsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    ...
    */
    public function collection()
    {
        ...
        foreach( $imports as $import) {
            ...
            $cards = CardsImports::select(\DB::raw('count(*) as num'))->groupBy('cards_imports.import_id')->get();

            if( count($import->cards) < $cards->max()->num )
            {
                foreach( $import->cards as $key => $item ) {
                    $all['card_name_'.$key]     = $item->name;
                }

                for ($i=1; $i <= ($cards->max()->num - count($import->cards))  ; $i++) { 
                    $all['card_name_'.(int)$i] = '';
                }
